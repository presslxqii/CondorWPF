        ...
        private void CreateSignalDict(Opc.Hda.Server Hda, List<string> tagsLst, string[] propID = null, string dtBegin = null, string dtEnd = null, bool restrict = false)
        {
            ...
            for (int i = 0; i < tagsLst.Count; i++)
            {
                ...
                if (dtBegin != null)
                {
                    ...
                    OpcRecord rec = null;
                    bool exit = false;
                     Opc.Hda.ItemValueCollection[] vForPrev = serverHda.ReadRaw(new Opc.Hda.Time(dt1), new Opc.Hda.Time(dt1.AddSeconds(1)), 0, true, items);
                    ...
                    Opc.Hda.ItemValueCollection[] vForNext = serverHda.ReadRaw(new Opc.Hda.Time(dt2.AddMilliseconds(-1)), new Opc.Hda.Time(dt2), 0, true, items);
                    ...
                    while (!exit)
                    {
                        Opc.Hda.ItemValueCollection[] values = serverHda.ReadRaw(new Opc.Hda.Time(lastdt), new Opc.Hda.Time(dt2), 0, false, items);
                        ...
                        for (int j = 0; j < values.Length; j++)
                        {
                            ...
                            for (int k = 0; k < values[j].Count; k++)
                            {
                                ...
                                if (values[j].Count > 1)
                                {
                                    if (k == values[j].Count - 1 && values[j][k].Timestamp == values[j][k - 1].Timestamp)
                                    {
                                ...
                                if (k != values[j].Count - 1)
                                {
                                    ...
                                    {
                                        if (System.Convert.ToDateTime(signal.ValueCollection[signal.ValueCollection.Count - 1].Timestamp).ToString() ==
